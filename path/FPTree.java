...
public class FPTree {
	...
	public static Logger logger = Logger.getLogger(FPTree.class);
		...
		if (filenames.length > 0) {
			...
			for (String filename : filenames) {
				try {
					...
					logger.info("Read transaction records failed." + ex.getMessage());
					System.exit(1);
				}
			}
		}
...
/*	private void combine(LinkedList<TreeNode> residualPath, List<List<TreeNode>> results) {
	...
	private void FPGrowth(List<List<String>> cpb, LinkedList<String> postModel) {
		...
		// logger.info(records);
		// }
		// logger.info("PostPattern is " + postPattern);
...
/*		String infile = "C:\\Users\\zzhou\\Desktop\\eclipse\\RecommendationFP-Tree\\src\\trolley1.txt";*/
		...
		/*
		 * List<List<String>> trans = fpTree.readTransRocords(new String[] {
		 * infile }); logger.info(trans);
		 */
		/* logger.info(trans.size()); */
		List<List<String>> trans1 = paste.dataProess();
/*		logger.info(trans1);*/
		Set<String> decideAttr = new HashSet<String>();
/*		decideAttr.add("cherokee");
		decideAttr.add("wrangler");
		decideAttr.add("compass");
		decideAttr.add("renegade");
		decideAttr.add("grand-cherokee");*/
		/*
		 * decideAttr.add("2500"); decideAttr.add("3500");
		 */
		fpTree.setDecideAttr(decideAttr);
		long begin = System.currentTimeMillis();
		fpTree.buildFPTree(trans1);
		long end = System.currentTimeMillis();
		logger.info("buildFPTree use time " + (end - begin));
...
/*		logger.info();*/
		...
		logger.info("条件\t结果\t支持度\t条件支持度");
		bw.write("条件\t结果\t支持度\t条件支持度");
		bw.newLine();
		DecimalFormat dfm = new DecimalFormat("#.####");
		for (StrongAssociationRule rule : rules) {
			logger.info(rule.condition + "->" + rule.result + "\t" + dfm.format(rule.support) + "\t"
